generate_journal_covid_inset_plot_first_author <- function(df, tiff = FALSE, end_year){
  ...
  covid_2020_counts <- df %>% 
    ...
    geom_segment(aes(y=0, yend=800, x=-Inf, xend=-Inf)) +
  ...
  p1 <- covid_plot_covid_only + 
    ...
    annotation_custom(
      ...
      xmin = lubridate::ymd("2019-02-01"), xmax = lubridate::ymd("2019-11-01"), ymin = 0, ymax = 40
    ) +
    annotation_custom(
      ggplotGrob(covid_2020_counts), 
      xmin = lubridate::ymd("2019-02-01"), xmax = lubridate::ymd("2019-11-01"), ymin = 45, ymax = 85
    ) +
  ...
  p2 <- covid_plot_noncovid_only + 
    ...
    annotation_custom(
      ...
      xmin = lubridate::ymd("2019-02-01"), xmax = lubridate::ymd("2019-11-01"), ymin = 50, ymax = 820
    ) +
    annotation_custom(
      ggplotGrob(noncovid_2021_counts), 
      xmin = lubridate::ymd("2020-02-01"), xmax = lubridate::ymd("2020-11-01"), ymin = 50, ymax = 820
    ) +
  ...
  ggsave("figs/JOURNAL_index_covid_plot_first_author.png")
  
  if(tiff){
    ggsave("figs/JOURNAL_index_covid_plot_first_author.tiff")
  }
  
  return("figs/JOURNAL_index_covid_plot_first_author.png")
}