...
gender_vs_year_counts <- model_data %>% 
  filter(date < lubridate::as_date("2021-01-01"), # Compare 2019 and 2020
...
## Proportion test
(year_test <- prop_z_test(gender_vs_year_matrix, correct = FALSE))
...
## Proportion test
(year_test_first <- prop_z_test(gender_vs_year_matrix_first, correct = FALSE))
...
gender_vs_year_counts_last <- model_data %>% 
  filter(order == "Last") %>% 
  filter(date < lubridate::as_date("2021-01-01"), date >= lubridate::as_date("2019-01-01")) %>% 
  mutate(year = lubridate::year(date)) %>% 
  group_by(year, gender) %>% 
  summarise(n = sum(n), .groups = "drop") %>% 
  drop_na() %>% 
...
## Proportion test
(year_test_last <- prop_z_test(gender_vs_year_matrix_last, correct = FALSE))
